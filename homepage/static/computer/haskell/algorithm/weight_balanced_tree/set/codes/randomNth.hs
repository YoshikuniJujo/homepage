...
main = do
	...
	time (nthS T.empty (randoms $ mkStdGen 8) 10000 :: Maybe Int)
	time (nthS T.empty (randoms $ mkStdGen 8) 20000 :: Maybe Int)
	time (nthS T.empty (randoms $ mkStdGen 8) 40000 :: Maybe Int)
...
nthS :: Ord a => T.Set a -> [a] -> Int -> Maybe a
nthS s (x : xs) n
	| x `T.member` s = nthS s xs n
	| n < 1 = Just x
	| otherwise = nthS (T.insert x s) xs (n - 1)
nthS _ _ _ = Nothing
